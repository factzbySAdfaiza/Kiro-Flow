...
async def verify_anthropic_api_key(
    ...
    Supports:
    1. Master PROXY_API_KEY (via x-api-key or Bearer)
    2. User-specific API keys from Persistence
    """
    # 1. Determine the provided key
    provided_key = None
    if x_api_key:
    ...
    # 2. Check master key
    if PROXY_API_KEY and provided_key == PROXY_API_KEY:
        return True
    
    # 3. Check persistence layer for user keys
    persistence = getattr(request.app.state, 'persistence', None)
    if persistence:
        key_info = await persistence.validate_api_key(provided_key)
        if key_info:
            # Attach user info to request.state
            request.state.current_user = key_info
            return True
            
    # 4. Fallback: Check if it's a direct Firebase ID token (Playground support)
    try:
        from firebase_admin import auth
        decoded_token = auth.verify_id_token(provided_key)
        if decoded_token:
            request.state.current_user = {"uid": decoded_token["uid"], "email": decoded_token["email"]}
            return True
    except Exception:
        pass

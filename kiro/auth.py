...
from kiro.persistence import get_persistence_provider
...
class KiroAuthManager:
    ...
    def __init__(
        ...
        self._persistence = get_persistence_provider()
        ...
        # Always try to load from persistence provider (this might overwrite SQLite/ENV if data exists)
        # We do this asynchronously in a hidden task or just allow it to be called
        # For now, we'll implement a sync-wrapper or call it at start of important methods
    ...
    async def load_from_persistence(self) -> None:
        """Loads credentials from the persistence provider."""
        try:
            ...
            logger.info("Credentials loaded from persistence provider")
            self._detect_auth_type()
        except Exception as e:
            logger.error(f"Error loading from persistence: {e}")

    async def save_to_persistence(self) -> None:
        """Saves current credentials to the persistence provider."""
        data = {
        ...
        await self._persistence.save(data)

    def _load_credentials_from_file(self, file_path: str) -> None:
        """
        Loads credentials from a JSON file (Deprecated in favor of persistence provider).
        """
        # Kept for compatibility but should use load_from_persistence
        pass
